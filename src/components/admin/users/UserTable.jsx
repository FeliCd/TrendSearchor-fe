...
function Pagination({ currentPage, totalPages, users, onPageChange }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-5 py-3 border-t border-white/[0.06]">
      <p className="text-xs text-[#8b949e]">Showing <span className="text-[#c9d1d9] font-semibold">{(currentPage - 1) * ITEMS_PER_PAGE + 1}–{Math.min(currentPage * ITEMS_PER_PAGE, users.length)}</span> of <span className="text-[#c9d1d9] font-semibold">{users.length}</span> users</p>
      <div className="flex items-center gap-1">
        <button onClick={() => onPageChange((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-3 py-1.5 rounded-lg text-xs font-medium text-[#8b949e] hover:text-white hover:bg-white/5 disabled:opacity-30 transition-all">Prev</button>
        ...
        <button onClick={() => onPageChange((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="px-3 py-1.5 rounded-lg text-xs font-medium text-[#8b949e] hover:text-white hover:bg-white/5 disabled:opacity-30 transition-all">Next</button>
      </div>
    </div>
  );
}
  ...
  useEffect(() => {
    if (openMenuId !== null) {
      ...
      if (btn) {
        ...
        if (menu) menu.style.setProperty('--menu-left', `${Math.max(8, rect.right - 176)}px`);
        if (menu) menu.style.setProperty('--menu-top', `${rect.bottom + 4}px`);
      }
    }
  }, [openMenuId]);
    ...
    <>
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 z-10 bg-[#0d1117]">
              <tr className="border-b border-white/[0.06]">
                {COLUMNS.map((h) => <th key={h} className="px-4 py-3.5 text-left text-[10px] font-bold uppercase tracking-widest text-[#8b949e] whitespace-nowrap first:pl-6 last:pr-6">{h}</th>)}
              </tr>
            </thead>
              ...
              {paginatedUsers.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-24 first:pl-6 last:pr-6">
                  <div className="flex flex-col items-center gap-3">
                    <div className="w-14 h-14 rounded-2xl bg-[#161b22] border border-white/[0.06] flex items-center justify-center"><Users className="w-6 h-6 text-[#8b949e]/40" /></div>
                    <div className="text-center"><p className="text-sm font-semibold text-[#c9d1d9]">No users found</p><p className="text-xs text-[#8b949e]/60 mt-0.5">Try adjusting your search or filters.</p></div>
                  </div>
                </td></tr>
              ) : (
                paginatedUsers.map((user, idx) => (
                  <motion.tr key={user.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2, delay: idx * 0.03 }}
                    ...
                    <td className="px-4 py-3.5 first:pl-6 last:pr-6">
                      <div className="flex items-center gap-3"><UserAvatar username={user.username} size="md" />
                        <div className="min-w-0"><p className="text-white font-semibold text-sm truncate max-w-[160px]">{user.username}</p><p className="text-xs text-[#8b949e] truncate max-w-[200px] mt-0.5">{user.mail}</p></div>
                      </div>
                    </td>
                    <td className="px-4 py-3.5 first:pl-6 last:pr-6">
                    ...
                    <td className="px-4 py-3.5 text-[#8b949e] whitespace-nowrap text-xs first:pl-6 last:pr-6" title={formatDate(user.lastLogin)}>{timeAgo(user.lastLogin)}</td>
                    <td className="px-4 py-3.5 text-[#8b949e] whitespace-nowrap text-xs hidden md:table-cell first:pl-6 last:pr-6">{formatDate(user.createdAt)}</td>
                    <td className="px-4 py-3.5 first:pl-6 last:pr-6">
                      <div className="flex justify-end">
                        <button ref={(el) => { menuButtonRefs.current[user.id] = el; }}
      ...
      <AnimatePresence>
        {openMenuId !== null && (() => {
          ...
          return user ? <ActionMenu key={openMenuId} user={user} onEdit={onEdit} onView={onView} onDelete={onDelete} onClose={() => setOpenMenuId(null)} /> : null;
        })()}
      </AnimatePresence>
    </>
  );
