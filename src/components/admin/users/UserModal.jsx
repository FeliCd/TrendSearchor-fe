...
const ICONS = {
  mail: <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>,
  lock: <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>,
  shield: <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>,
  chart: <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>,
  user: <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>,
};
...
export default function UserModal({ user, mode, onClose, onSave }) {
  ...
  const [form, setForm] = useState({ mail: '', password: '', fullName: '', role: ROLES.LECTURER, status: USER_STATUS.ACTIVE });
  ...
  useEffect(() => {
    if (user) {
      const role = user.role === ROLES.STUDENT ? ROLES.LECTURER : (user.role || ROLES.LECTURER);
      setForm({ mail: user.mail || '', password: '', fullName: user.fullName || '', role, status: user.status || USER_STATUS.ACTIVE });
    }
    else setForm({ mail: '', password: '', fullName: '', role: ROLES.LECTURER, status: USER_STATUS.ACTIVE });
  }, [user]);
          ...
          <FormField label="Role" icon={ICONS.shield}>
            {isView ? (
              <div className="space-y-1.5">
                <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold ${ROLE_COLORS[form.role]?.badgeDark || ''}`}>{RoleIcon[form.role]}{form.role === ROLES.LECTURER ? 'Student / Lecturer' : ROLE_LABELS[form.role] || form.role}</span>
                <p className="text-xs text-gray-400">{DESCS[form.role]}</p>
              </div>
            ) : (
              <SelectDropdown
                value={form.role}
                ...
                options={Object.values(ROLES)
                  .filter(r => r !== ROLES.STUDENT && r !== ROLES.USER && (isCreate || r !== ROLES.ADMIN || form.role === ROLES.ADMIN))
            ...
            <FormField label="Status" icon={ICONS.chart}>
              {isView ? (
                <div className="space-y-1.5">
                  ...
                  {user?.lastLogin && <p className="text-xs text-gray-400">Last login: {timeAgo(user.lastLogin)}</p>}
                  {user?.createdAt && <p className="text-xs text-gray-400">Since: {formatDate(user.createdAt)}</p>}
                </div>
              ) : (
                <SelectDropdown
                  value={form.status}
