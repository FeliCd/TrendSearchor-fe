...
import { KEYWORD_COLORS, CHART_TOOLTIP_STYLE } from '@/constants/chartConfig';
...
export default function TopicComparisonChart({ comparison }) {
  if (!comparison || !comparison.yearlyDataMap || !comparison.keywords?.length) {
    return <div className="h-64 flex items-center justify-center text-sm text-gray-500">Search and select keywords to compare</div>;
  }

  const { yearlyDataMap, keywords } = comparison;
  const allYears = new Set();
  keywords.forEach((kw) => (yearlyDataMap[kw.toLowerCase()] || []).forEach((d) => d.year && allYears.add(d.year)));
  const sortedYears = [...allYears].sort((a, b) => a - b);
  const chartData = sortedYears.map((year) => {
    const row = { year };
    keywords.forEach((kw) => {
      const entry = (yearlyDataMap[kw.toLowerCase()] || []).find((d) => d.year === year);
    ...
    <div className="space-y-3">
      ...
      <ResponsiveContainer width="100%" height={280}>
        <LineChart data={chartData} margin={{ top: 5, right: 10, bottom: 5, left: -10 }}>
          ...
          <XAxis dataKey="year" tick={{ fontSize: 11, fill: '#6b7280' }} tickLine={false} axisLine={{ stroke: '#ffffff10' }} />
          ...
          {keywords.map((kw, i) => (
            <Line key={kw} type="monotone" dataKey={kw} stroke={KEYWORD_COLORS[i % KEYWORD_COLORS.length]}
              strokeWidth={2} dot={{ r: 3, fill: KEYWORD_COLORS[i % KEYWORD_COLORS.length], strokeWidth: 0 }} activeDot={{ r: 5 }} connectNulls />
          ))}
        </LineChart>
      </ResponsiveContainer>
