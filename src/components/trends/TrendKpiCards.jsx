...
export function KpiCard({ label, value, sub, color }) {
  return (
    <div className="bg-[#1e1e1e] border-2 border-gray-800 rounded-none p-4">
...
export function TrendKpiCard({ keyword, analysis, compact = false }) {
  ...
  if (compact) {
    return (
      <div className={`p-2.5 rounded-none border ${config.border} ${config.bg}`}>
  ...
  return (
    <div className={`rounded-none border ${config.border} ${config.bg} p-4`}>
      ...
      <div className="space-y-2">
        ...
        {analysis?.forecastNextYear != null && (
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-medium text-[#0058be]">{analysis.forecastNextYear >= 0 ? '+' : ''}{Math.round(analysis.forecastNextYear * 100)}%</span>
          </div>
        )}
      </div>
    </div>
