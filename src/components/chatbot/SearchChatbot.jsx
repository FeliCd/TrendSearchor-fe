    ...
    <>
      ...
      <div className="fixed bottom-6 right-3 z-50">
        <button
          ...
          className={`flex items-center justify-center w-12 h-12 bg-gradient-to-r from-[#0058be] to-[#004395] text-white hover:from-[#004395] hover:to-[#0058be] shadow-[0_0_15px_rgba(0,88,190,0.5)] transition-all duration-300 border border-[#0058be]/40 hover:scale-105 active:scale-95 ${isOpen ? 'rotate-90' : ''
            }`}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-3 w-[360px] h-[480px] max-h-[78vh] bg-[#151515] border-2 border-gray-800 shadow-2xl flex flex-col z-50 overflow-hidden"
          >
            {/* Header */}
            <div className="px-3.5 py-2 border-b-2 border-gray-800 bg-[#1a1a1a]">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <div className="w-6 h-6 bg-[#0058be]/10 border border-[#0058be]/50 flex items-center justify-center flex-shrink-0">
                    <Sparkles className="w-3.5 h-3.5 text-[#0058be]" />
                  </div>
                  <h3 className="text-xs font-black uppercase tracking-wider text-white truncate">AI Search Assistant</h3>
                </div>
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button
                    onClick={handleClearChat}
                    className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-white hover:bg-[#282828] transition-colors rounded"
              ...
              <div className="flex items-center gap-1.5 mt-0.5 pl-8">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping"></span>
                <span className="text-[9px] text-gray-500 font-bold uppercase tracking-wider">Online</span>
              </div>
            </div>

            {/* Chat Messages */}
            <div data-lenis-prevent className="flex-1 overflow-y-auto p-3 space-y-2.5 scrollbar-dark">
              {messages.map((msg) => {
                if (msg.sender === 'user') {
                  return (
                    <div key={msg.id} className="flex justify-end">
                      <div className="bg-[#0058be]/15 border border-[#0058be]/40 text-gray-100 max-w-[75%] w-fit px-2.5 py-1.5 text-[11px] leading-[1.45] select-text">
                        {msg.text}
                      </div>
                    </div>
                  );
                ...
                if (msg.type === 'loading') {
                  return (
                    <div key={msg.id} className="flex gap-2">
                      <div className="w-6 h-6 bg-gray-800 border border-gray-700 flex items-center justify-center flex-shrink-0">
                        <Sparkles className="w-3 h-3 text-[#0058be]" />
                      </div>
                      <div className="bg-[#1e1e1e] border border-gray-800 text-gray-300 max-w-[75%] w-fit px-2.5 py-1.5 text-[11px] leading-[1.45] flex items-center gap-1.5">
                        <Loader2 className="w-3 h-3 animate-spin text-[#0058be]" />
                        <span>{msg.text}</span>
                      </div>
                    </div>
                  );
                ...
                if (msg.type === 'results') {
                  ...
                  return (
                    <div key={msg.id} className="flex gap-2">
                      ...
                      <div className="flex-1 space-y-2 min-w-0 max-w-[75%]">
                        <div className="bg-[#1e1e1e] border border-gray-800 text-gray-300 px-2.5 py-1.5 text-[11px] leading-[1.45] w-fit max-w-full">
                          {msg.text}
                        </div>
                        ...
                        <div className="space-y-2">
                          {papers.length === 0 ? (
                            <div className="p-2.5 border border-dashed border-gray-850 text-center text-[11px] text-gray-500 bg-[#1a1a1a]">
                            ...
                            papers.map((paper) => (
                              <div
                                ...
                                className="bg-[#1c1c1c] border border-gray-800 hover:border-gray-700 p-2.5 flex flex-col justify-between gap-1.5"
                              >
                                <div>
                                  <h4 className="text-[11.5px] font-bold text-white line-clamp-2 leading-snug">
                                    {paper.title}
                                  </h4>
                                  ...
                                  {paper.aiRelevanceScore && (
                                    <div className="flex items-center justify-between bg-[#0058be]/10 border border-[#0058be]/30 px-2 py-0.5 mt-1 text-[9.5px]">
                                      ...
                                      <span className="text-gray-400 truncate max-w-[150px] text-[8.5px]" title={paper.aiRelevanceReason}>
                                        {paper.aiRelevanceReason}
                                      </span>
                                    </div>
                                  )}
                                </div>
                                <div className="flex items-center justify-between mt-1 pt-1 border-t border-gray-850">
                                  <span className="text-[9.5px] bg-gray-800 text-gray-400 px-1.5 py-0.5 border border-gray-750">
                                    {paper.year || 'N/A'}
                                  </span>
                                  <button
                        ...
                        {papers.length > 0 && canNavigateToSearch && (
                          <button
                            ...
                            className="w-full flex items-center justify-center gap-1.5 py-1.5 border border-gray-800 hover:border-[#0058be] bg-[#1e1e1e] hover:bg-[#0058be]/5 text-[10.5px] text-gray-300 hover:text-white font-bold uppercase tracking-wider transition-all"
                          >
                ...
                if (msg.type === 'recommendations') {
                  ...
                  return (
                    <div key={msg.id} className="flex gap-2">
                      <div className="w-6 h-6 bg-[#0058be] border border-[#4A90E2] flex items-center justify-center flex-shrink-0 shadow-lg shadow-[#0058be]/20">
                        <Sparkles className="w-3 h-3 text-white animate-pulse" />
                      </div>
                      <div className="flex-1 space-y-2 min-w-0 max-w-[75%]">
                        <div className="bg-[#1e1e1e] border border-[#0058be]/50 text-gray-200 px-2.5 py-1.5 text-[11px] leading-[1.45] shadow-sm w-fit max-w-full">
                          <div className="flex items-center gap-1 text-[9px] font-black uppercase tracking-wider text-[#4A90E2] mb-1">
                        ...
                        <div className="space-y-2">
                          {recommendations.map((rec, idx) => (
                            <div
                              key={idx}
                              className="bg-[#1c1c1c] border border-gray-800 hover:border-[#0058be]/60 p-2.5 transition-all flex flex-col justify-between gap-1.5 shadow-sm"
                            >
                              <div>
                                <div className="flex items-center justify-between gap-1.5">
                                  ...
                                  <span className="text-[8.5px] font-black uppercase tracking-wider px-1.5 py-0.5 bg-[#282828] text-gray-400 border border-gray-750">
                                    {rec.type || 'TOPIC'}
                                  </span>
                                </div>
                                <p className="text-[10.5px] text-gray-300 mt-1.5 leading-relaxed bg-[#141414] p-2 border-l-2 border-[#0058be]">
                                  "{rec.reason}"
                                </p>
                              </div>
                              <div className="flex items-center justify-end gap-2 pt-1 border-t border-gray-850">
                                <button
                                  onClick={() => handleSend(null, `Find papers about ${rec.name}`)}
                                  className="px-2 py-1 bg-[#242424] hover:bg-[#0058be] text-gray-300 hover:text-white text-[9px] font-bold uppercase tracking-wider transition-colors flex items-center gap-1"
                                >
                                  <Search className="w-2.5 h-2.5" /> Search Papers
                                </button>
                              </div>
                            </div>
                          ))}
                ...
                return (
                  <div key={msg.id} className="flex gap-2">
                    ...
                    <div className="bg-[#1e1e1e] border border-gray-800 text-gray-300 max-w-[75%] w-fit px-2.5 py-1.5 text-[11px] leading-[1.45] whitespace-pre-line select-text">
                      {msg.text}
                    </div>
                  </div>
                );
            ...
            {/* Input Bar */}
            <form onSubmit={handleSend} className="p-2 border-t border-gray-800 bg-[#1a1a1a] flex gap-1.5 items-stretch">
              <input
                type="text"
                value={input}
                ...
                className="flex-1 h-8 bg-[#151515] border border-gray-800 px-2.5 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-[#0058be] transition-colors"
              />
              {loading ? (
                <button
                  type="button"
                  onClick={handleStopGeneration}
                  className="px-2.5 h-8 bg-red-600/80 hover:bg-red-600 text-white flex items-center justify-center gap-1 text-[11px] font-bold uppercase tracking-wider transition-colors border border-red-500/50 flex-shrink-0"
                ...
                <button
                  type="submit"
                  disabled={!input.trim()}
                  className="w-8 h-8 bg-[#0058be] text-white hover:bg-[#004395] disabled:opacity-50 disabled:hover:bg-[#0058be] flex items-center justify-center transition-colors flex-shrink-0"
                  title="Send Message"
                >
