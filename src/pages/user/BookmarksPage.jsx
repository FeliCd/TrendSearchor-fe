    ...
    {[{ k: 'ALL', l: 'All', Ic: null }, { k: 'PAPER', l: 'Papers', Ic: null }, { k: 'KEYWORD', l: 'Keywords', Ic: null }].map(({ k, l }) => (
      <button key={k} onClick={() => onFilterChange(k)}
        className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-1.5 ${
          filter === k ? 'bg-[#4A90E2] text-white' : 'bg-[#161b22] text-gray-400 hover:text-white border border-white/10'
        }`}>{l}
      </button>
    ))}
  </div>
...
const Pagination = ({ page, totalPages, onPageChange }) => (
  <div className="flex justify-center gap-2 mt-6">
    <button onClick={() => onPageChange(Math.max(0, page - 1))} disabled={page === 0}
      className="px-4 py-2 bg-[#161b22] border border-white/10 text-white rounded-lg disabled:opacity-30 hover:border-white/20 transition-colors">Previous</button>
    <span className="px-4 py-2 text-gray-400 text-sm">Page {page + 1} of {totalPages}</span>
    <button onClick={() => onPageChange(page + 1)} disabled={page >= totalPages - 1}
      className="px-4 py-2 bg-[#161b22] border border-white/10 text-white rounded-lg disabled:opacity-30 hover:border-white/20 transition-colors">Next</button>
  </div>
);
