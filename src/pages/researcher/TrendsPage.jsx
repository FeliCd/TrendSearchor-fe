...
import { TrendingUp, BarChart2, Loader2, Bookmark, TrendingDown, Minus } from 'lucide-react';
...
export default function TrendsPage() {
  ...
  const loadKeywords = useCallback(async () => {
    try {
      const data = await trendService.getTrendingKeywords(30);
      setKeywords(data);
                ...
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg transition-colors ${
                    selectedKeyword === kw.keyword
                      ? 'bg-[#4A90E2]/20 border border-[#4A90E2]/40 text-white'
                      : 'bg-[#0d1117] border border-transparent text-gray-300 hover:bg-[#0d1117]/80'
                  }`}
                >
